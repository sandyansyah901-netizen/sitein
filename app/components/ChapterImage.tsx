...
export default function ChapterImage({
  ...
  const handleLoadSuccess = () => {
    ...
    setIsLoaded(true);
    console.log(`✅ [${pageOrder}/${totalPages}] ✨ LOADED in ${completeTime - loadStart}ms`);
  ...
  const handleLoadError = () => {
    ...
    setHasError(true);
    console.error(`❌ [${pageOrder}/${totalPages}] 💥 FAILED after ${errorTime - loadStart}ms`);
  ...
  return (
    <div className="relative w-full">
      {/* Loading placeholder — hanya tampil saat belum loaded, tinggi dinamis */}
      {!isLoaded && !hasError && (
        <div className="flex flex-col items-center justify-center bg-card-bg" style={{ minHeight: "60vh" }}>
          ...
          <p className="text-sm font-semibold text-foreground">📥 Memuat Halaman {pageOrder}</p>
      ...
      {/* Gambar: block display penting agar tidak ada inline whitespace */}
      <img
        src={imageUrl}
        alt={`Page ${pageOrder}`}
        className={`block w-full transition-opacity duration-300 ${isLoaded ? "opacity-100" : "opacity-0 absolute top-0 left-0"
          }`}
        loading="eager"
        decoding="async"
      ...
      {hasError && (
        <div
          className="flex flex-col items-center justify-center bg-card-bg border border-red-900/20"
          style={{ minHeight: "60vh" }}
        >
          <svg className="mb-3 h-14 w-14 text-red-500/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              strokeLinecap="round"
          ...
          <p className="mb-1 text-sm font-semibold text-foreground">💥 Gagal Memuat Halaman {pageOrder}</p>
          <p className="mb-2 text-xs text-muted">Coba periksa koneksi internet</p>
          <p className="mb-4 text-[10px] text-red-400/70 max-w-xs truncate px-4">{imageUrl}</p>
          <button
            onClick={handleRetry}
