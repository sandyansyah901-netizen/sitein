...
interface SearchBarProps {
  /** Ukuran search bar: "default" (halaman utama) | "compact" (header) */
...
export default function SearchBar({ variant = "default", onClose }: SearchBarProps) {
  ...
  const fetchSuggestions = useCallback(async (q: string) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/v1/manga/suggest?q=${encodeURIComponent(q)}&limit=5`
      );
      if (!res.ok) throw new Error("Gagal fetch suggest");
  ...
  // Submit form → navigasi ke halaman hasil
  ...
  // Klik salah satu suggestion → ke halaman detail manga
  ...
  return (
    <div ref={containerRef} className={`relative ${isCompact ? "w-full" : "w-full max-w-xl"}`}>    
      ...
      {showDropdown && suggestions.length > 0 && (
        <div
          className={`absolute left-0 right-0 z-50 mt-1 rounded-xl border border-border bg-white dark:bg-[#111] shadow-lg overflow-hidden ${
            isCompact ? "top-full" : "top-full"
          }`}
        >
          <ul>
              ...
              <li key={s.slug}>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-semibold text-foreground truncate dark:text-white">
                      {s.title}
                    </p>
          ...
          {/* Footer: tekan Enter untuk cari semua */}
